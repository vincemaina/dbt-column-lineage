...
Honest limitation: a source column never referenced (or referenced ambiguously without qualification in a
multi-table query) won't be derivable; such columns degrade to `unknown`.
"""
...
_INFERRED_TYPE = "INFERRED"  # placeholder — lineage resolution needs column names, not types
...
class InferredSchemaResolver:
    ...
    # -- pass 1: derive source columns from usage --------------------------------------------------

    def _non_model_relations(self) -> set[str]:
        relations = {n.relation.key() for n in self._artifacts.sources.values()}
        relations |= {
            n.relation.key() for n in self._artifacts.nodes.values() if n.resource_type != "model"
        }
        return relations

    def _derive_source_columns(self, source_relations: set[str]) -> SchemaMapping:
        usage: dict[str, set[str]] = defaultdict(set)
        for node in self._artifacts.nodes.values():
            if node.resource_type != "model" or not node.compiled_code:
                continue
            try:
                parsed = parse_one(node.compiled_code, dialect=self._dialect)
            except SqlglotError:
                continue
            self._collect_source_columns(parsed, source_relations, usage)
        return {rel: {c: _INFERRED_TYPE for c in cols} for rel, cols in usage.items() if cols}

    @staticmethod
    def _collect_source_columns(
        parsed: exp.Expression, source_relations: set[str], usage: dict[str, set[str]]
    ) -> None:
        # alias -> source relation_key, for base tables that are sources
    ...
    # -- pass 2: infer model schemas in DAG order --------------------------------------------------

    def _compute(self) -> None:
        derived = self._derive_source_columns(self._non_model_relations())
        ...
        for uid in self._topological_models():
            node = self._artifacts.nodes[uid]
            if node.resource_type != "model" or not node.compiled_code:
                continue
            relation = node.relation.key()
            columns = self._infer_columns(node.compiled_code, accumulated)
            if columns:
                column_map = {c.upper(): _INFERRED_TYPE for c in columns}
    ...
    def _infer_columns(self, compiled_sql: str, schema) -> list[str]:
        try:
            qualified = qualify(
                parse_one(compiled_sql, dialect=self._dialect),
                schema=schema,
    ...
    def _topological_models(self) -> list[str]:
        ...
        parents = {
            u: {p for p in self._artifacts.parent_map.get(u, ()) if p in models} for u in models
        }
        ...
        seen = set(order)
        order.extend(
            u for u in sorted(models) if u not in seen
        )  # cycle remnants, deterministically
        return order
